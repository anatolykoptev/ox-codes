use ox_dataflow::{Finding, FindingKind, analysis, scope_walker};
...
fn has(findings: &[Finding], var: &str, kind: &FindingKind) -> bool {
    findings
        .iter()
        .any(|f| f.variable == var && f.kind == *kind)
}
...
fn go_dead_store() {
    ...
    assert!(
        has(&f, "x", &FindingKind::DeadStore),
        "x := 1 should be dead store, got: {f:?}"
    );
}
...
fn go_underscore_skip() {
    ...
    assert!(
        bad.is_empty(),
        "_ and _unused should be skipped, got: {bad:?}"
    );
}
...
fn go_const_values() {
    ...
    for var in ["x", "y", "z"] {
        assert!(
            has(&f, var, &FindingKind::ConstantValue),
            "{var} should have const value"
        );
        assert!(
            has(&f, var, &FindingKind::DeadStore),
            "{var} should be dead store"
        );
    }
}
...
fn go_empty_function() {
    ...
    assert!(
        warnings.is_empty(),
        "empty function should have no findings, got: {warnings:?}"
    );
}
...
fn go_multiple_functions() {
    ...
    assert!(
        !has(&f, "x", &FindingKind::UnusedVariable),
        "x in foo() is used"
    );
    // y in bar() is unused
    assert!(
        has(&f, "y", &FindingKind::DeadStore),
        "y in bar() should be dead store, got: {f:?}"
    );
}
...
fn var_has_use(src: &str, lang: &str, name: &str) -> bool {
    ...
    chain
        .scopes
        .iter()
        .any(|s| s.vars.iter().any(|v| v.name == name && !v.uses.is_empty()))
}
