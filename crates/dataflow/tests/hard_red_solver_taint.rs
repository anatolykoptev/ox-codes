...
use ox_dataflow::taint::{analyze_taint, Sanitizer, TaintFinding, TaintSink, TaintSource};
use ox_dataflow::taint_rules::{default_rules, TaintRule};
...
fn go_taint(s: &str, r: &[TaintRule]) -> Vec<TaintFinding> {
    ...
    il.functions.iter().flat_map(|f| analyze_taint(&build_cfg(f), r, "t.go")).collect()
}
...
fn sqli_rule() -> TaintRule {
    TaintRule {
        id: "sqli".into(),
        sources: vec![TaintSource { pattern: "FormValue".into(), tag: "i".into() }],
        sinks: vec![TaintSink {
            pattern: "Query".into(), arg_index: 0,
            cwe: "CWE-89".into(), description: "sqli".into(),
        }],
        sanitizers: vec![], severity: "error".into(),
    }
}
...
fn redef_same_block_only_latest_reaches_use() {
    ...
    assert!(first.uses.is_empty(), "x:=1 killed by x=2: {:?}", first.uses);
}

#[test]
fn both_branch_defs_collected() {
    let src = "package main\nfunc f(){\nif true{x:=1;_=x}else{x:=2;_=x}\n}";
    let n = collect_definitions(&go_cfg(src)).iter().filter(|d| d.name.ident == "x").count();
...
fn unused_var_empty_uses() {
    let c = build_def_use_chains(&go_cfg("package main\nfunc f(){x:=42}")).unwrap();
    assert!(c.iter().find(|c| c.def.name.ident == "x").unwrap().uses.is_empty());
}

#[test]
fn var_used_twice_both_captured() {
    let c = build_def_use_chains(&go_cfg("package main\nfunc f(){x:=1;y:=x+x;_=y}")).unwrap();
    assert!(c.iter().find(|c| c.def.name.ident == "x").unwrap().uses.len() >= 2);
}
...
fn self_assignment_first_def_used() {
    ...
    let x: Vec<_> = c.iter().filter(|c| c.def.name.ident == "x").collect();
    assert!(x.len() >= 2);
    assert!(x.iter().any(|c| !c.uses.is_empty()), "first x def used in x=x+1 RHS");
}
...
fn transitive_chain_x_to_y_to_z() {
    ...
    assert!(!c.iter().find(|c| c.def.name.ident == "x").unwrap().uses.is_empty());
    assert!(!c.iter().find(|c| c.def.name.ident == "y").unwrap().uses.is_empty());
}

#[test]
fn no_defs_empty_chains() {
    assert!(build_def_use_chains(&go_cfg("package main\nfunc f(){}")).unwrap().is_empty());
}
...
fn taint_via_string_concat() {
    let s = "package main\nfunc f(){\ninput:=FormValue(\"x\")\nq:=\"SELECT \"+input\nQuery(q)\n}";
    assert!(!go_taint(s, &[sqli_rule()]).is_empty(), "concat propagates taint");
}

#[test]
fn direct_nested_source_in_sink_no_panic() {
    let _ = go_taint("package main\nfunc f(){Query(FormValue(\"x\"))}", &[sqli_rule()]);
}
...
fn multiple_sources_all_reach_sinks() {
    let r = TaintRule {
        id: "m".into(),
        sources: vec![
            TaintSource { pattern: "FormValue".into(), tag: "a".into() },
            TaintSource { pattern: "ReadAll".into(), tag: "b".into() },
        ],
        sinks: vec![TaintSink {
            pattern: "Query".into(), arg_index: 0,
            cwe: "CWE-89".into(), description: "s".into(),
        }],
        sanitizers: vec![], severity: "error".into(),
    };
    let s = "package main\nfunc f(){\na:=FormValue(\"x\")\nb:=ReadAll(body)\nQuery(a)\nQuery(b)\n}";
    assert!(go_taint(s, &[r]).len() >= 2, "both sources should reach sinks");
}
...
fn same_func_source_and_sink() {
    let r = TaintRule {
        id: "s".into(),
        sources: vec![TaintSource { pattern: "Query".into(), tag: "d".into() }],
        sinks: vec![TaintSink {
            pattern: "Query".into(), arg_index: 0,
            cwe: "CWE-89".into(), description: "s".into(),
        }],
...
fn underscore_taint_no_panic() {
    let _ = go_taint("package main\nfunc f(){_=FormValue(\"x\");Query(_)}", &[sqli_rule()]);
}
...
fn custom_rules_not_defaults() {
    let c = TaintRule {
        id: "custom-xss".into(),
        sources: vec![TaintSource { pattern: "UserInput".into(), tag: "c".into() }],
        sinks: vec![TaintSink {
            pattern: "Render".into(), arg_index: 0,
            cwe: "CWE-79".into(), description: "x".into(),
        }],
        sanitizers: vec![Sanitizer { pattern: "Escape".into() }],
        severity: "warning".into(),
    };
    let s = "package main\nfunc f(){\ndata:=UserInput(\"f\")\nRender(data)\n}";
    let f = go_taint(s, &[c]);
    assert!(!f.is_empty() && f[0].rule_id == "custom-xss");
    assert!(go_taint(s, &default_rules("go")).is_empty(), "defaults shouldn't match");
}
...
fn dead_store_overwrite() {
    ...
    assert!(f.iter().any(|f| f.kind == FindingKind::DeadStore && f.variable == "x"), "{f:?}");
}
...
fn underscore_prefix_skipped() {
    ...
    assert!(!f.iter().any(|f| f.kind == FindingKind::DeadStore && f.variable.starts_with('_')));
}
